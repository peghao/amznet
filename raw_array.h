...
typedef struct raw_array array;
...
void array_add_constand(float *t1, float *t, size_t size, float c)
{
    for(size_t i=0; i<size; i++)
        t[i] = t1[i] + c;
}
...
void array_add_broad(float *t1, float *t2, float *t, size_t num_row, size_t row_size)
{
    for(size_t i=0; i<num_row; i++){
        array_add_constand(t1+i*row_size, t+i*row_size, row_size, t2[i]);
    }
}
...
void array_times_constand_original(float *t1, float *t, size_t size, float c)
{
    #pragma omp parallel for num_threads(4)
    for(size_t i=0; i<size; i++)
        t[i] = t1[i] * c;
...
#ifdef ACC_AVX
void array_times_constand_avx(float *t1, float *t, size_t size, float c)
{
    #define BATCHSIZE 8
    size_t num_batch = size/BATCHSIZE;
    size_t mod = size % BATCHSIZE;
...
void array_times_constand(float *t1, float *t, size_t size, float c)
{
    for(size_t i=0; i<size; i++)
        t[i] = t1[i] * c;
}
...
void array_div_constand(float *t1, float *t, size_t size, float c)
{
    for(size_t i=0; i<size; i++)
        t[i] = c/t1[i];
}
...
{
    for(size_t i=0; i<num_row; i++)
        array_div_constand(t1+i*row_size, t+i*row_size, row_size, t2[i]);
}
